...
export async function parseIntent(message) {
  ...
  // Local Rule-Based Parsing (To save Gemini API Quota)
  if (clean === "summarize mails" || clean === "summarize emails" || clean === "summarise emails" || clean === "summarise mails") {
    return { intent: "summarize_emails", query: "", sender: null, timeframe: null };
  }
  if (clean === "action items" || clean === "pending action items" || clean === "show pending action items" || clean === "todo" || clean === "tasks") {
    return { intent: "action_items", query: "", sender: null, timeframe: null };
  }
  if (clean === "important emails" || clean === "important mails" || clean === "show important emails" || clean === "show important mails") {
    return { intent: "important_emails", query: "", sender: null, timeframe: null };
  }
  if (clean === "daily report" || clean === "give me my daily report" || clean === "show daily report") {
    return { intent: "daily_report", query: "", sender: null, timeframe: null };
  }
  if (clean === "what meetings do i have today" || clean === "meetings today" || clean === "todays meetings" || clean === "today meetings" || clean === "my schedule today") {
    return { intent: "todays_meetings", query: "", sender: null, timeframe: null };
  }
  if (clean === "upcoming meetings" || clean === "upcoming meeting" || clean === "schedule upcoming") {
    return { intent: "upcoming_meetings", query: "", sender: null, timeframe: null };
  }
  if (clean === "recent meetings" || clean === "show recent meetings" || clean === "show recent meeting" || clean === "recent meeting") {
    return { intent: "recent_meetings", query: "", sender: null, timeframe: null };
  }

  // Handle simple "search emails from [sender]"
  const fromSenderMatch = clean.match(/^search (?:emails|mails) from (.+)$/) || 
                           clean.match(/^(?:emails|mails) from (.+)$/);
  if (fromSenderMatch) {
    const senderName = fromSenderMatch[1].trim();
    return { intent: "search_emails", query: "", sender: senderName, timeframe: null };
  }

  // Handle simple "last email from [sender]"
  const lastEmailFromMatch = clean.match(/^(?:what is the |show |get )?last (?:email|mail) from (.+)$/);
  if (lastEmailFromMatch) {
    const senderName = lastEmailFromMatch[1].trim();
    return { intent: "search_emails", query: "last email", sender: senderName, timeframe: null };
  }
