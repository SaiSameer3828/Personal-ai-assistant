...
export async function downloadWhatsAppMedia(mediaId) {
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(`WhatsApp media lookup failed: ${JSON.stringify(error)}`);
  }

  const mediaInfo = await response.json();
  if (!mediaInfo.url) {
    throw new Error("No media URL found in Meta response.");
  }
  ...
  if (!downloadResponse.ok) {
    throw new Error(`WhatsApp media download failed: ${downloadResponse.status}`);
  }
