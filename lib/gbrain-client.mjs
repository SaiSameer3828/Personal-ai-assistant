...
// Dynamically determine WSL home directory once at startup on Windows
let wslHomeDir = "/home/sai_sameer";
if (IS_WINDOWS) {
  try {
    wslHomeDir = execSync('wsl bash -c "echo ~"', { encoding: "utf8" }).trim();
  } catch (err) {
    console.error("Failed to get WSL home directory dynamically:", err.message);
  }
}

const GBRAIN_WSL_PATH = `${wslHomeDir}/.bun/bin/gbrain`;
const BUN_PATH = `${wslHomeDir}/.bun/bin`;
...
const GBRAIN_ENV = { 
  ...
  // Only override HOME and disable Bun WASM interpreter on Linux/Docker (Railway)
  ...(IS_WINDOWS ? {} : { HOME: "/tmp", BUN_JSC_useWasmIPInt: "false" })
};
 ...
 */
export function runGbrain(cmd) {
  if (IS_WINDOWS) {
    // On Windows, run through WSL using execFileSync to preserve quotes and arguments
    return execFileSync("wsl", [
      "bash",
      "-c",
      `export PATH="${BUN_PATH}:/usr/bin:/bin:\$PATH" && export GEMINI_API_KEY='${config.gemini.apiKey}' && export GOOGLE_API_KEY='${config.gemini.apiKey}' && export GOOGLE_GENERATIVE_AI_API_KEY='${config.gemini.apiKey}' && gbrain ${cmd}`
    ], {
      encoding: "utf8",
      timeout: 15000,
    });
...
function runGbrainPut(slug, filePath) {
  if (IS_WINDOWS) {
    ...
    execFileSync("wsl", [
      "bash",
      "-c",
      `export PATH="${BUN_PATH}:/usr/bin:/bin:\$PATH" && export GEMINI_API_KEY='${config.gemini.apiKey}' && export GOOGLE_API_KEY='${config.gemini.apiKey}' && export GOOGLE_GENERATIVE_AI_API_KEY='${config.gemini.apiKey}' && gbrain put ${slug} < '${wslPath}'`
    ], {
      stdio: "ignore",
      timeout: 10000,
    });
...
export function documentExists(slug) {
  try {
    runGbrain(`get ${slug.toLowerCase()}`);
    return true;
  } catch {
    return false;
  }
